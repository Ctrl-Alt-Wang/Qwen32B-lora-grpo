...
| 参数 | v3 | **v4** | 说明 |
|------|----|--------|------|
| `lora_r` | 8 | **16** | LoRA rank 翻倍，可学习参数量约翻倍 |
| `lora_alpha` | 16 | **32** | 随 r 等比调整，scaling factor = alpha/r = 2.0（不变） |
...
| WandB run name | `qwen32b-lora-r16-rollout4-v4` |
| WandB run ID | `4rusviq8` |
...
| 启动脚本 | `run_32b_lora_grpo_trl.sh`（`LORA_R=16 LORA_ALPHA=32 ... bash run_32b_lora_grpo_trl.sh`） |
